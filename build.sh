...
# Increment the patch version and create new version strings
NEW_PATCH=$((PATCH + 1))
NEW_VERSION_STRING="${MAJOR}.${MINOR}.${NEW_PATCH}"
NEW_VERSION_TAG="v${NEW_VERSION_STRING}"
FLUTTER_VERSION_STRING="${NEW_VERSION_STRING}+${NEW_PATCH}" # Format for pubspec.yaml (e.g., 1.0.3+3)

echo "✅ New version will be: ${NEW_VERSION_TAG}"

# 2. Update pubspec.yaml
echo "📝 Updating pubspec.yaml to version ${FLUTTER_VERSION_STRING}..."
sed -i "s/^version: .*/version: ${FLUTTER_VERSION_STRING}/" pubspec.yaml

# 3. Commit all current changes, including the version update
...
# 5. Create the new GitHub Release
